...
export class MessagesGateway implements OnGatewayConnection {
  ...
  async handleMessage(client: Socket, payload: { text: string; receiverId: number }) {
    const senderId = (client as any).userId;
    const msg = await this.messagesService.create(payload.text, senderId, payload.receiverId);
